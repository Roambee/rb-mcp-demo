...
async function handleMessageSubmit(e) {
    ...
    // Track the number of messages before this request
    const messagesBefore = document.querySelectorAll('.message.assistant').length;
    ...
    try {
        const result = await retryRequest(async () => {
            ...
            if (data.status === 'error') {
                throw new Error(data.detail || 'Server returned error response');
            }
        ...
        // Check if we added a new assistant message during this request
        const messagesAfter = document.querySelectorAll('.message.assistant').length;

        // Only show error message if no new assistant message was added
...
function showNotification(message, type = 'info') {
    // Simple console notification for now - can be enhanced later
    console.log(`[${type.toUpperCase()}] ${message}`);

    // Optional: Show browser alert for critical notifications
